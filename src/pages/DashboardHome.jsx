...
export default function DashboardHome() {
  ...
  const handleUploadFile = async () => {
    ...
    try {
      ...
      alert(`Помилка при завантаженні: ${err.response?.data?.detail || 'Перевірте формат файлу'}`);
    } finally {
  ...
  const handleDeleteFile = async (fileId) => {
      ...
      try {
        ...
        const textToDisplay = typeof errorMsg === 'object' 
  ...
  const handleDeleteCase = async (caseId) => {
    if (!window.confirm('Видалити медичну справу та всі прикріплені до неї файли? Цю дію неможливо скасувати.')) return;
    ...
    try {
      ...
      alert(`Помилка видалення: ${err.response?.data?.detail || 'Ендпоінт недоступний або немає прав'}`);
    }
  };
  ...
  const handleUpdateRecord = async () => {
    ...
    try {
      await api.put(`/api/records/cases/${editRecord.id}`, {
        ...
        description: editRecord.description?.trim() === '' ? null : editRecord.description,
  ...
  const RenderFiles = ({ records }) => {
    if (!records || records.length === 0) return <Typography variant="body2" color="text.secondary">—</Typography>;
    return (
        ...
        <Box>
          <Paper sx={{ p: 3, borderRadius: 2, mb: 4, borderLeft: '5px solid #1976d2' }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Typography variant="body1">Відкрийте новий медичний запис, щоб ваш лікар міг призначити лікування.</Typography>
              <Button variant="contained" startIcon={<AddIcon />} onClick={() => setOpenCreateModal(true)}>Відкрити справу</Button>
            </Box>
          </Paper>
              ...
              <TableBody>
                {patientHistory.map((caseItem) => (
                  <TableRow key={caseItem.id}>
                    <TableCell>{new Date(caseItem.created_at).toLocaleDateString('uk-UA')}</TableCell>
                    <TableCell fontWeight="bold">{caseItem.case_name}</TableCell>
                    <TableCell>{caseItem.treatment || 'Очікує лікаря'}</TableCell>
                    <TableCell><RenderFiles records={caseItem.records} /></TableCell>
                    <TableCell align="right">
                      <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
                        <IconButton color="primary" onClick={() => openUpload(caseItem.id)} title="Завантажити аналізи">
                          <CloudUploadIcon />
                        </IconButton>
                        <IconButton color="error" onClick={() => handleDeleteCase(caseItem.id)} title="Видалити справу">
                          <DeleteOutlinedIcon />
                        </IconButton>
                      </Box>
                    </TableCell>
          ...
          <Dialog open={openCreateModal} onClose={() => setOpenCreateModal(false)} maxWidth="sm" fullWidth>
            ...
            <DialogContent dividers>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 1 }}>
                <TextField label="Коротко про проблему" fullWidth required variant="outlined" value={newRecord.case_name} onChange={(e) => setNewRecord({...newRecord, case_name: e.target.value})} />
                <TextField label="Детальний опис симптомів" fullWidth multiline rows={3} variant="outlined" value={newRecord.description} onChange={(e) => setNewRecord({...newRecord, description: e.target.value})} />
              </Box>
            </DialogContent>
          ...
          <Dialog open={openUploadModal} onClose={() => setOpenUploadModal(false)} maxWidth="xs" fullWidth>
            ...
            <DialogContent dividers>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 1 }}>
                <TextField label="Назва документа (напр. Рентген)" fullWidth required variant="outlined" value={uploadData.title} onChange={(e) => setUploadData({...uploadData, title: e.target.value})} />
                <Button variant="outlined" component="label" fullWidth startIcon={<AttachFileIcon />} sx={{ height: '56px', textTransform: 'none', justifyContent: 'flex-start', px: 2 }}>
            ...
            <DialogActions sx={{ p: 2 }}>
              <Button onClick={() => setOpenUploadModal(false)} color="inherit">Скасувати</Button>
              <Button onClick={handleUploadFile} variant="contained" disabled={!uploadData.title || !uploadData.file || uploadLoading}>
        ...
        <Box>
          <Paper sx={{ p: 3, borderRadius: 2, mb: 4, boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}>
            ...
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: '16px' }}>
              <TextField label="ID Пацієнта" variant="outlined" size="small" type="number" value={searchId} onChange={(e) => setSearchId(e.target.value)} sx={{ width: '250px' }} />
              <Button type="submit" variant="contained" color="success" disabled={searchLoading || !searchId} startIcon={searchLoading ? <CircularProgress size={20} /> : <SearchIcon />}>
                Знайти картку
              </Button>
            </form>
          ...
          {searchedPatientId && (
            <Box>
              <Typography variant="h6" fontWeight="bold" sx={{ mb: 2 }}>Медична історія (Пацієнт ID: {searchedPatientId})</Typography>
              <TableContainer component={Paper} sx={{ borderRadius: 2 }}>
                <Table>
                  <TableHead sx={{ bgcolor: '#f1f5f9' }}>
                    <TableRow>
                      ...
                      <TableCell sx={{ fontWeight: 'bold' }}>Скарги</TableCell>
                      <TableCell sx={{ fontWeight: 'bold' }}>Призначення</TableCell>
                      <TableCell sx={{ fontWeight: 'bold' }}>Документи пацієнта</TableCell>
                      <TableCell align="right" sx={{ fontWeight: 'bold' }}>Дія</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {patientHistory.map((caseItem) => (
                      <TableRow key={caseItem.id}>
                        <TableCell>{new Date(caseItem.created_at).toLocaleDateString('uk-UA')}</TableCell>
                        <TableCell fontWeight="bold">{caseItem.case_name}</TableCell>
                        <TableCell>{caseItem.treatment || <Typography color="error" variant="body2">Немає</Typography>}</TableCell>
                        <TableCell><RenderFiles records={caseItem.records} /></TableCell>
                        <TableCell align="right">
                          <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 1 }}>
                            <Button size="small" variant="contained" color={caseItem.treatment ? "info" : "success"} startIcon={<EditIcon />} onClick={() => openEdit(caseItem)}>
          ...
          <Dialog open={openEditModal} onClose={() => setOpenEditModal(false)} maxWidth="sm" fullWidth>
            ...
            <DialogContent dividers>
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, mt: 1 }}>
                <TextField label="Уточнений діагноз" fullWidth variant="outlined" value={editRecord.case_name} onChange={(e) => setEditRecord({...editRecord, case_name: e.target.value})} />
                <TextField label="Симптоми пацієнта (можна змінити)" fullWidth multiline rows={2} variant="outlined" value={editRecord.description} onChange={(e) => setEditRecord({...editRecord, description: e.target.value})} />
                <TextField label="Лікування (Рецепт)" fullWidth multiline rows={3} required variant="outlined" color="success" focused value={editRecord.treatment} onChange={(e) => setEditRecord({...editRecord, treatment: e.target.value})} />
              </Box>
            </DialogContent>
            <DialogActions sx={{ p: 2 }}>
